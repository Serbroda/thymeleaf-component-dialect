...
public class FragmentHelper {
	...
	public static IModel getFragmentModel(final ITemplateContext context,
		...
		if (fragmentObj == null) {
			...
			// If the Fragment result is NO-OP, we will just do nothing (apart
			// from deleting the th:* attribute)
			// return;

		} else if (fragmentObj == Fragment.EMPTY_FRAGMENT) {

			// The result is the empty fragment, which means we simply have to
			// either remove the body of this
			// tag (th:insert) or remove it completely, tag included
			// (th:replace)
			// if (this.replaceHost) {
		...
		/*
		 * CHECK WHETHER THIS IS A CROSS-TEMPLATE-MODE INSERTION. Only
		 * TemplateModels for the same template mode can be safely inserted into
		 * the template being executed and processed just like any other
		 * sequences of events. If the inserted template has a different
		 * template mode, we will need to process it aside and obtain a String
		 * result for it, then insert such String as mere text.
		 *
		 * Note inserting large templates with a different template mode could
		 * therefore have a negative effect on performance and memory usage, as
		 * their result needs to be completely stored in memory at some point
		 * before being handled to the following phases of template processing.
		 * It is therefore recommended that cross-template-mode fragment
		 * insertion is done only for small fragments, in which case it will
		 * work almost the same as inlining (with the exception that the content
		 * to be inlined will be retrieved from somewhere else by means of
		 * template resolution).
		 */
		if (context.getTemplateMode() != fragmentModel.getTemplateMode()) {
			...
			// "Template being processed uses template mode " +
			// context.getTemplateMode() + ", " +
			// "inserted fragment \"" + attributeValue + "\" uses template mode
			// " +
			// fragmentModel.getTemplateMode() + ". Cross-template-mode fragment
			// insertion is not " +
			// "allowed using the " + attributeName + " attribute, which is no
			// longer recommended for use as " +
			// "of Thymeleaf 3.0. Use {th:insert,data-th-insert} or
			// {th:replace,data-th-replace} " +
			// "instead, which do not remove the container element from the
			// fragment being inserted.");
			// }

			// If there are parameters specified, we will need to add them
			// directly to the variables map instead of
			// doing it through the structure handler (we are going to perform a
			// nested template processing operation)
				...
				if (!(context instanceof IEngineContext)) {
					throw new TemplateProcessingException(
					    "Parameterized fragment insertion is not supported because local variable support is DISABLED. This is due to "
					            + "the use of an implementation of the "
					            + ITemplateContext.class.getName()
					            + " interface that does "
					            ...
					            + IEngineContext.class.getName()
					            + " interface");
				}

				// NOTE this IEngineContext interface is internal and should not
				// be used in users' code
				((IEngineContext) context).setVariables(fragmentParameters);

			}

			// Once parameters are in order, just process the template in a
			// nested template engine execution
			final Writer stringWriter = new FastStringWriter(200);
			configuration.getTemplateManager().process(fragmentModel, context,
			    stringWriter);

			// We will insert the result as NON-PROCESSABLE text (it's already
			// been processed!)
			// if (this.replaceHost) {
		...
		/*
		 * IF WE ARE ASKING ONLY FOR CONTENTS (th:include), THEN REMOVE THE
		 * CONTAINER BLOCK
		 */
		if (fragmentTemplateData.hasTemplateSelectors()) {

			/*
			 * In the case of th:include, things get a bit complicated because
			 * we need to remove the "element envelopes" that contain what we
			 * really want to include (these envelopes' contents). So we will
			 * need to traverse the entire returned model detecting those
			 * envelopes (open+close tags at model level == 0) and remove them,
			 * along with anything else that is also at that level 0.
			 */

			final IModel model = fragmentModel.cloneModel();
			int modelLevel = 0;
			int n = model.size();
			while (n-- != 0) { // We traverse backwards so that we can modify at
			                   // the same time
				...
				if (event instanceof ICloseElementTag) {
					if (((ICloseElementTag) event).isUnmatched()) {
						// This is an unmatched close tag (no corresponding
						// open), therefore should not affect our count
						continue;
					}
					if (modelLevel <= 0) {
						model.remove(n);
	...
	public static Object computeFragment(final ITemplateContext context,
	        final String input) {
		...
		if (shouldBeWrappedAsFragmentExpression(trimmedInput)) {
			// We do not know for sure that this is a complete standard
			// expression, so we will consider it the
			// content of a FragmentExpression for legacy compatibility reasons.
			// We will only reach this point if the expression does not contain
			// any Fragment Expressions expressed
			// as ~{...} (excluding parameters), nor the "::" fragment selector
			// separator.
			// NOTE we are using the generic parseExpression() and not directly
			// calling a parse method in the
			// FragmentExpression class because we want to take advantage of the
			// expression cache.
			final FragmentExpression fragmentExpression =
			        (FragmentExpression) expressionParser
			            .parseExpression(context, "~{" + trimmedInput + "}");
			...
			if (executedFragmentExpression
			    .getFragmentSelectorExpressionResult() == null
			        && executedFragmentExpression
			            .getFragmentParameters() == null) {
				// We might be in the scenario that what we thought was a
				// template name in fact was instead an expression
			...
			// Given this is a simple (originally unwrapped) fragment
			// expression, we will consider the non-existence
			// of the fragment a failure. The reason we do this here instead of
			// just waiting and seeing if we receive
			// a null and then failing is that, in order to receive such "null",
			// the underlying resolution system would
			// have to execute a (potentially costly) resource.exists() call on
		...
		// If we reached this point, we know for sure this is a complete
		// fragment expression, so we just parse it
		// as such and execute it

		final IStandardExpression fragmentExpression =
		        expressionParser.parseExpression(context, trimmedInput);

		final Object fragmentExpressionResult;

		if (fragmentExpression != null
		        && fragmentExpression instanceof FragmentExpression) {
			// This is not a complex expression but merely a FragmentExpression,
			// so we can apply a shortcut
			// so that we don't require a "null" result for this expression if
			// the template does not exist. That will
		...
		if (fragmentExpressionResult == null
		        || fragmentExpressionResult == NoOpToken.VALUE) {
	...
	static boolean shouldBeWrappedAsFragmentExpression(final String input) {
		...
		if (inputLen > 2 && input.charAt(0) == FragmentExpression.SELECTOR
		        && input.charAt(1) == '{') {
			// This input already starts as a fragment expression, so we are
			// sure there is no need to wrap
			return false;
		}
		char c;
		int bracketLevel = 0;
			...
			if (c == '\'') {
				...
				if (c == '{') {
					...
					if (c == '(') {
						...
						// In exactly this disposition (paramLevel == 1,
						// bracketLevel == 0), we know we are looking at
						// a named parameter in a FragmentExpression content, so
						// this has to be wrapped
						return true;
					} else if (c == FragmentExpression.SELECTOR && n != 0
					        && input.charAt(i + 1) == '{') {
						// A fragment expression appears at level 0, so this
						// should not be wrapped
						return false;
					} else if (c == ':' && n != 0
					        && input.charAt(i + 1) == ':') {
						// A fragment selector ("::") has been found, so this is
						// the content of a fragment expression and
		...
		// We haven't been able to determine any useful information about the
		// type of expression this might be so,
		// in practice, this will mean we will consider it a fragment expression
		// content for legacy compatibility
		// reasons (afterwards we will rectify if what we consider a "template
		// name" happens to be a Fragment).
		return true;
